 ...
 */
export function migrateLegacyApiKeyAuthToProviders({ lingxiHome, providerRegistry, log = () => {} }: { lingxiHome: string; providerRegistry: any; log?: (msg: string) => void }) {
  if (!lingxiHome) return { migrated: 0, providers: [] };

  const authPath = path.join(lingxiHome, "auth.json");
  ...
  const modelsJsonProvidersRaw = readJson(path.join(lingxiHome, "models.json")).providers || {};
