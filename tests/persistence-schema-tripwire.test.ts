...
describe("persistence schema tripwire", () => {
  it("uses real SQLite stores and matches the deterministic committed fingerprint", async () => {
    ...
    expect(facts.runtimeSchema.objects.some((entry) => entry.name === "fact_embeddings")).toBe(true);
    ...
    for (const table of [
      ...
      "knowledge_research_actions", "knowledge_completeness_checks", "knowledge_completeness_units",
  ...
  it("ignores JSDoc drift, including the file headers this guard once taxed", async () => {
    ...
    const markers: Array<[string, string]> = [
      ["server/index.ts", "LingxiAgent Server — HTTP + WebSocket API"],
