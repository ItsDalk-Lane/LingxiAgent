...
it("真实知识入口的来源召回全部命中，HNSW 热查询只定点读块", async () => {
  ...
  try {
    ...
    for (const [index, topic] of topics.entries()) {
      const result = await manager.searchService.searchWithEvidence({ compiledScope, query: `${topic} 规定`, channel: "hybrid",
        limit: 1, rerankPolicy: KNOWLEDGE_RERANK_DISABLED_POLICY });
