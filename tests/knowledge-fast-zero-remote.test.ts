...
async function fixture() {
  ...
  cleanups.push(async () => { await manager.close(); fs.rmSync(home, { recursive: true, force: true }); });
  ...
  let currentPath = "";
  const session = { getActiveToolNames: () => ["knowledge_search", "knowledge_read", "knowledge_grep", "knowledge_outline"], model: null, subscribe: vi.fn(() => () => {}), sessionManager: { appendCustomEntry: vi.fn() } };
  ...
  const engine = Object.assign(Object.create(LingxiEngine.prototype), {
    ...
    getSessionIdForPath: vi.fn((value: string) => { currentPath = value; return "session"; }),
    getSessionManifest: vi.fn(() => ({ lifecycle: "active", currentLocator: { path: currentPath } })),
...
describe("连续查阅准备及保留的本地快速入口零远程调用", () => {
  it("发送只冻结范围；本地快速入口仍能产生精确证据，均不触碰远程入口", async () => {
    ...
    expect(event.message.knowledgeRetrieval).toMatchObject({ executionPath: "conversation", retrievalMode: "none", injectedChunks: 0 });
    expect(engine.promptSession.mock.calls[0][1]).not.toContain("发布必须完成审批。");
    expect(engine.promptSession.mock.calls[0][1]).toContain("knowledge_search");
    const manifest = manager.getEvidenceManifestByScope({ scopeId: event.message.knowledgeRetrieval.scopeId });
    expect(manifest.entries).toHaveLength(0);
    const fast = await engine.buildFastKnowledgeContext({ question: "审批", sessionPath: "/tmp/local-fast.jsonl", turnId: "local-fast", knowledgeRefs: { notebookIds: [notebook.id], mode: "fast" } });
    expect(fast.stats).toMatchObject({ executionPath: "fast_local", remoteModelCalls: 0, vectorQueries: 0, rerankCalls: 0, ftsQueries: 1, injectedChunks: 1 });
    expect(fast.block).toContain("发布必须完成审批。");
    expect(fast.evidence.entries).toHaveLength(1);
    expect(fast.evidence.entries[0].blockSpans).toHaveLength(1);
