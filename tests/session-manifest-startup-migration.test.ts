...
describe("session manifest startup migration", () => {
  ...
  beforeEach(() => {
    lingxiHome = fs.mkdtempSync(path.join(os.tmpdir(), "hana-manifest-startup-"));
    nextId = 1;
  ...
  it("creates a rollback checkpoint before migrating legacy sessions once", () => {
    ...
    expect(first.result).toEqual({ scanned: 1, created: 1, existing: 0, skipped: 0, skippedDetails: [], skippedMetaSources: [] });
