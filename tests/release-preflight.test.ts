...
describe("release preflight", () => {
  ...
  it("passes against all fetched tags that belong to the current product", () => {
    expect(runReleasePreflight({ candidateTag: "v0.1.41" })).toMatchObject({
      candidateVersion: "0.1.41",
      historicalMaximumVersion: "0.1.40-experimental.1",
      historicalMaximumGeneration: 18,
      candidateGeneration: 19,
    });
  });
});
