...
describe("win32 sandbox runtime cache", () => {
  ...
  it("mirrors bundled Git under LINGXI_HOME and rewrites all runtime paths", () => {
    ...
    expect(path.relative(lingxiHome, prepared.git).startsWith("..")).toBe(false);
  });

  it("mirrors the Node executable directory under LINGXI_HOME", () => {
    const tempRoot = makeTempRoot();
    const lingxiHome = path.join(tempRoot, "home");
    ...
    expect(path.relative(lingxiHome, prepared.executable).startsWith("..")).toBe(false);
  });
