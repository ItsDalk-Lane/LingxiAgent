...
describe("全局融合后单次重排", () => {
  it("五个笔记本同一引用只调用一次，全局 FTS 也只有一次", async () => {
    ...
    const localFts = vi.spyOn(data.manager.indexStore, "searchReadyVariantIds");
    const legacyFts = vi.spyOn(data.manager.indexStore, "search");
    const result = await data.manager.searchService.search(data.request);
    expect(localFts).toHaveBeenCalledTimes(1); expect(legacyFts).not.toHaveBeenCalled();
