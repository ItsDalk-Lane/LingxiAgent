...
- **手动安装**：将插件目录放到 `${LINGXI_HOME}/plugins/`。实际目录可在设置 → 插件页面或 `/api/plugins/settings` 的 `plugins_dir` 查看
...
插件私有数据自动存放在 `${LINGXI_HOME}/plugin-data/{pluginId}/`。删除插件时此目录保留，重新安装后配置还在。
...
**SKIP 链**：同一事件类型可以注册多个 handler。系统按注册顺序调用，直到某个 handler 返回非 `LINGXI_BUS_SKIP` 的值。返回 `LINGXI_BUS_SKIP` 表示"我不处理，交给下一个"：
...
如果已安装版本高于当前 app 可兼容的最高市场版本，安装动作会被标记为 `downgrade`，必须显式传 `allowDowngrade: true` 才能继续。拖拽 / 本地路径安装同样会阻止隐式降级。更新安装会先备份旧目录到 `${LINGXI_HOME}/plugin-backups/<pluginId>/`，新版本加载失败时恢复旧目录并重新加载；成功后 `${LINGXI_HOME}/plugin-installs.json` 会记录来源、版本、release URL 和 sha256，供后续市场状态判断。
