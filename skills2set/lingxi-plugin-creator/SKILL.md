...
# Lingxi Plugin Creator

Use this skill for Lingxi application plugins, not Codex `.codex-plugin` bundles.

## First Contact

On first use, give a map, not an encyclopedia. Explain what Lingxi plugins can add, ask what the user wants to build, and invite follow-up questions. Expand details only after the user asks or after the chosen scaffold needs them.
...
Beginner mode tone: encouraging, concrete, and guided. Say that the user can describe the feature in plain language, LingxiAgent will help turn it into a plugin plan and scaffold, and LingxiAgent can answer questions at any step. Ask:

1. 你希望 LingxiAgent 多一个什么能力？
...
After delivering a plugin, encourage with grounded product value. Name the real situation where the plugin helps, such as reducing repeated steps, making an external service available inside Lingxi, turning a manual workflow into an Agent-callable tool, or giving a recurring task a stable UI. Use natural wording such as `这个想法挺实用，适合把每周重复整理的步骤固定下来` or `这个方向比较适合做成工具型插件，因为 Agent 可以在对话里直接调用`. Avoid inflated praise like `你的设想太棒了`.

## Capability Map

Lingxi plugins can provide:

- Agent-callable tools and slash-style actions.
- Skills, agents, and knowledge that guide model behavior.
- WebView/iframe pages, widgets, and cards using Lingxi theme and host capabilities.
...
- Session and Agent control through `@lingxi/plugin-runtime`: create/list/update/send/abort/history sessions, subscribe to session events, create/read/update plugin-owned agents, and hide plugin-private resources from the main Lingxi UI.
...
Lingxi provides install/enable/reload, per-agent skill toggles, manifest capability checks, iframe host messaging, theme tokens, toast/clipboard/external host APIs, EventBus, data directories, and SDK packages.
...
- It finds Python through `LINGXI_PLUGIN_CREATOR_PYTHON`, `python3`, `python`, or Windows `py -3`.
...
1. Find the Lingxi repo root. Prefer the current workspace if it contains `PLUGIN_SDK.md`, `PLUGINS.md`, and `packages/plugin-runtime`.
...
3. Pick a template:
   ...
   - `guided-react`: React/Vite/SDK starter with shared Lingxi components and a gentler README.
...
5. Pick the target location:
   ...
   - User-installed plugin: the directory reported by `/api/plugins/settings` or `${LINGXI_HOME}/plugins`.
6. Generate the scaffold with the bundled script, then adjust names, descriptions, tools, routes, capabilities, and UI to the user's request.
   - When converting an existing website or single-page app into a Lingxi iframe plugin, rewrite same-plugin browser `fetch('/api/...')` calls to `lingxi.api.fetch(...)`, move static files under `assets/`, and use `lingxi.assets.url(...)` for browser-side asset references.
...
- React templates may use `@lingxi/plugin-runtime`, `@lingxi/plugin-sdk`, and `@lingxi/plugin-components`.
- Static iframe resources belong under `assets/` and should be referenced with `lingxi.assets.url(path)` from browser code or the official `/api/plugins/{pluginId}/assets/...` path from the route shell. This includes CSS, JS, images, fonts, JSON, wasm, and browser-playable videos such as MP4/WebM/MOV. Do not inline large assets as a workaround.
...
- Dev authority is not a manifest permission. Lingxi grants it from the remembered dev install slot under `${LINGXI_HOME}/plugins-dev/`, and Agent dev tools are hidden until the user enables the dev tools setting.
...
- Iframe browser code must not call third-party APIs directly. It should call a same-plugin route with `lingxi.api.fetch(...)`; the route reads config/secrets server-side and calls `ctx.network.fetch(...)`.
...
- Browser iframe code may call `lingxi.resources.open()`, `lingxi.resources.pick()`, or `lingxi.resources.requestAccess()` as host-mediated requests only. It must not read or write filesystem content directly; server-side plugin routes and tools should use `ctx.resources` for actual resource operations.
- If iframe code calls `lingxi.resources.open()`, `lingxi.resources.pick()`, or `lingxi.resources.requestAccess()`, declare the matching `resource.open`, `resource.pick`, or `resource.requestAccess` entry under `ui.hostCapabilities`. Do not request resource host grants in templates that do not use those calls.
...
- `createSession()` creates a detached Lingxi session and does not switch the main UI focus. Use `visibility: "plugin_private"` and `ownerPluginId` for plugin-only sessions or Tavern-style parallel chat surfaces.
- Use `createChatSurfaceCard(ctx, session.sessionRef ?? session, options)` when a plugin wants to display its own private session in chat. Do not hand-build path-only `chat.surface` payloads; the helper requires `sessionId` / `sessionRef` and Lingxi verifies plugin ownership before rendering.
...
- After full-access plugin install, enable, or reload, Lingxi rebinds extension runners for idle sessions. Busy sessions pick up the change on the next safe rebuild, so do not promise that an in-flight reply will use freshly edited extension code.
- Declare only the iframe host capabilities actually used.
- EventBus handlers should return `LINGXI_BUS_SKIP` for payloads that do not belong to them.
...
- For a single release, root `version`, `compatibility`, and `distribution` remain valid; Lingxi normalizes them into a single version entry.
- Lingxi selects the highest SemVer version compatible with the current app and exposes update, reinstall, incompatible, and downgrade states to the UI.
...
- Default React plugin UI to `LingxiThemeProvider mode="inherit"` so it follows the host theme.
- Use `mode="lingxi"` for a named Lingxi theme, and `mode="custom"` only for explicit token overrides.
...
- Direct templates include `lingxi.api.fetch()` for plugin route calls; preserve that helper when simplifying generated browser code.
...
- Live data APIs belong behind plugin routes that call `ctx.network.fetch()`. The browser page calls `lingxi.api.fetch('api/...')`, receives sanitized JSON, and renders it locally.
- First paint must not auto-call LLM APIs. Trigger model calls only from an explicit user action, then call a plugin backend route with `lingxi.api.fetch(...)`; the backend route may use `sampleText()` or other runtime helpers.
