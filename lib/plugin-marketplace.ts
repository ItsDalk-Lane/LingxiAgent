...
export function createDefaultPluginMarketplace({ lingxiHome, env = process.env, fetchImpl }: any = {}) {
  const envPath = normalizeOptionalText(env.LINGXI_PLUGIN_MARKETPLACE_FILE);
  const envUrl = normalizeOptionalText(env.LINGXI_PLUGIN_MARKETPLACE_URL);
  const localIndex = lingxiHome ? path.join(lingxiHome, "plugin-marketplace", "marketplace.json") : null;
