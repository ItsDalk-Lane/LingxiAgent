...
export class KnowledgeSearchService {
  ...
  async searchWithEvidence(request: KnowledgeSearchRequest, sectionsBySourceId?: ReadonlyMap<string, string[]>): Promise<{
    ...
    const cached = await this.resultCache.getOrCreate({
      ...
      request = { ...request, query: normalizeKnowledgeQuery(request.query), rerankPolicy, signal };
      ...
      if (request.channel === "hybrid") {
        ...
        if (request.rerankPolicy.enabled && candidates.length > 0) {
      ...
      return {
        ...
        evidence: {
          ...
          rerankDegradeReasons,
          rerankSkippedReasons,
