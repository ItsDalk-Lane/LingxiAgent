...
async function soakHistoryCache(batch, cache, sessionPaths) {
  ...
  fs.appendFileSync(appendPath, JSON.stringify({ type: "message", id: `soak-append-${batch}`, parentId: null, timestamp: "2026-09-22T00:00:00Z", message: { role: "user", content: `soak append ${batch}` } }) + "\n", "utf8");
