...
let bootNumber = 0;
const OLD_TREE = "/var/folders/5r/wq54gtms21gfjgr5v94ynfyh0000gn/T/lingxi-f05-old-i2x_43qi";
const argv = process.argv.slice(2);
let outPath = path.join(EVIDENCE, "result.json");
...
function boot(treeRoot, home, entry) {
  ...
  children.add(child);
  const rawLog = path.join(EVIDENCE, `server-${++bootNumber}.log`);
  child.stdout.on("data", c => fs.appendFileSync(rawLog, c));
  child.stderr.on("data", c => fs.appendFileSync(rawLog, c));
  let tail = "";
...
try {
  ...
  allOk = false;
  record("DRILL", "异常退出", "无异常", error?.stack || error, "FAIL");
} finally {
  for (const child of children) {
    if (child.exitCode === null) { child.kill("SIGTERM"); await waitExit(child); }
  }
...
const summary = {
  tool: "F05 source-level upgrade-rollback drill (local protocol witness, not installed artifact)",
