...
const result=JSON.parse(fs.readFileSync(`${out}/soak-result.json`));const session={path:result.sessions[29],agentId:'lingxi'}; await task(session,'SOAK_BROWSER_APPROVAL_WITNESS'); await api('browser/close-session',{sessionPath:session.path});
