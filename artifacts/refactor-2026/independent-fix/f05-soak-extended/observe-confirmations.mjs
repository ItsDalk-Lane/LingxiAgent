...
const ws=new WebSocket(`ws://127.0.0.1:${info.port}/ws`,{headers});
ws.on('message',raw=>{const event=JSON.parse(raw);if(event.type==='confirmation_resolved'||event.block?.type==='session_confirmation')log('confirmation-wire',{event});});
