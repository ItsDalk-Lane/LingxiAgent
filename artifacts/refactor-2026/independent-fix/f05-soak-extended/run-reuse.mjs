...
await sample('reuse-baseline');
const fixed=JSON.parse(fs.readFileSync(`${out}/soak-result.json`)).sessions.slice(-6);
const sessions=[];
for(let batch=1;batch<=2;batch++){
 const batchSessions=[];
 for(let slot=1;slot<=6;slot++){
  const cycle=(batch-1)*6+slot;
  const session={path:fixed[slot-1],agentId:'lingxi'};
 ...
 await sample('reuse-batch-'+batch+'-open');
 await sleep(16000);
 await sample('reuse-batch-'+batch+'-settled-open');
 for(let slot=1;slot<=6;slot++){
 ...
 await sample('reuse-batch-'+batch+'-closed');
 await sleep(18000);
 await sample('reuse-batch-'+batch+'-settled-closed');
}
fs.writeFileSync(`${out}/reuse-result.json`,JSON.stringify({start:new Date(started).toISOString(),end:new Date().toISOString(),elapsedMs:Date.now()-started,cycles:12,status:'completed',sessions:sessions.map(s=>s.path)},null,2));
