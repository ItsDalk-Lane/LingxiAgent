...
const result=JSON.parse(fs.readFileSync(`${out}/soak-result.json`));
await sample('recovery-tail-1');await sleep(18000);await sample('recovery-tail-2');await sleep(18000);await sample('recovery-tail-3');
log('tail-complete',{originalStart:result.start});
