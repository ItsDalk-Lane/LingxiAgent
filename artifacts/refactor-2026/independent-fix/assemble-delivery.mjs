...
const full=read(A+'/main/full-tests-recheck-delivery.json');
const commandIndex=A+'/main/commands.jsonl';
const commands=fs.readFileSync(commandIndex,'utf8').trim().split('\n').map(JSON.parse);
const fullCommand=commands.find(c=>c.id==='engineering-full-tests-recheck');
...
const fullEv=[A+'/main/engineering-full-tests-recheck.out',A+'/main/engineering-full-tests-recheck.err',A+'/main/full-tests-recheck-delivery.json'];
const recheckDir=A+'/recheck-r1-r2';
const recheckEv=[recheckDir+'/r1-red.out',recheckDir+'/r1-red.err',recheckDir+'/r2-red.out',recheckDir+'/r2-red.err',recheckDir+'/F03_RECHECK_RESULT.json',recheckDir+'/commands.jsonl',recheckDir+'/redaction-audit.md',A+'/main/recheck-f03-regression.out',A+'/main/recheck-affected-surface.out',A+'/main/recheck-typecheck.out',A+'/main/recheck-strict-contracts.out',A+'/main/recheck-lint-scope.out',A+'/main/recheck-ws-protocol.out'];
...
// recheck 轮（R1/R2）：接纳原子性与恢复代次两个已确认缺陷的修复证据并入 C14—C19 族。
const recheck=read(recheckDir+'/F03_RECHECK_RESULT.json');
for(const case_ of regressions){
  const update=(recheck.case_updates||[]).find(u=>u.id===case_.id);
  if(!update) continue;
  case_.evidence=[...new Set([...(case_.evidence||[]),...recheckEv])];
  case_.recheck_round={round:'recheck-r1-r2',date:recheck.date,note:update.note,see:recheckDir+'/F03_RECHECK_RESULT.json'};
}
