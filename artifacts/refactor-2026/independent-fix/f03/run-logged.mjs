#!/usr/bin/env node
/**
 * P05 命令日志记录器（2026-09-22 引入；与 P00/P02 工具同构）。
 * 用法：node run-logged.mjs --id <command_id> --reason "<原因>" [--cwd <dir>] [--retry-of <id>] -- <argv...>
 * 行为：执行 argv，stdout/stderr 原样写入 artifacts/refactor-2026/P05/logs/<id>.{out,err}，
 * 追加全格式 JSONL 条目（argv/cwd/source_sha/起止时间/exit_code/status/digest）。
 * 子进程 exit code 原样透传。条目格式与 P04 修复轮后的全格式一致（含时间戳/digest）。
 */
...
const LOG_DIR = path.join(ROOT, "artifacts", "refactor-2026", "independent-fix", "f03");
...
const entry = {
  command_id: id, stage_id: "F03", cwd,
