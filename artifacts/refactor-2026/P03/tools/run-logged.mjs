#!/usr/bin/env node
/**
 * P03 命令日志记录器（复验收修复轮 2026-09-22 引入；与 P00/P02 工具同构）。
 * 用法：node run-logged.mjs --id <command_id> --reason "<原因>" [--cwd <dir>] [--retry-of <id>] -- <argv...>
 * 行为：执行 argv，stdout/stderr 原样写入 artifacts/refactor-2026/P03/logs/<id>.{out,err}，
 * 追加全格式 JSONL 条目（argv/cwd/source_sha/起止时间/exit_code/status/digest）。
 * 子进程 exit code 原样透传。此前 P03 既有条目为弱格式（无时间戳/digest），
 * 属已登记退化（P03_REPORT 复验收修复轮 #4）；历史条目不重建、不改动。
 */
...
const LOG_DIR = path.join(ROOT, "artifacts", "refactor-2026", "P03", "logs");
...
const entry = {
  command_id: id, stage_id: "P03", cwd,
