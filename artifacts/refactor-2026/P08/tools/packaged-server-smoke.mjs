...
try {
  ...
  let tail = "";
  child.stdout.on("data", (c) => { tail += String(c); });
  child.stderr.on("data", (c) => { tail += String(c); });
...
const summary = { tool: "P08 packaged-server-smoke", seed: seedDir, pass: results.filter((r) => r.status === "PASS").length, fail: results.filter((r) => r.status === "FAIL").length, all_pass: allOk, results };
