#!/usr/bin/env node
/**
 * P00-T06 内存工作负载初始样本（受限口径）：隔离 HOME 启动 server，
...
const home = path.join(P00, "isolated", "bench-mem-A");
...
const child = spawn(process.execPath, ["server/main-full.ts"], {
  cwd: REPO, env: { ...process.env, LINGXI_HOME: home, LINGXI_TOKEN: "p00-bench-mem" }, stdio: "ignore",
});
...
for (let i = 0; i < 200; i++) {
  const r = await globalThis.fetch(`http://127.0.0.1:${info.port}/api/input-drafts`, {
    method: "PUT", headers: { "content-type": "application/json", authorization: "Bearer p00-bench-mem" },
...
const out = path.join(P00, "samples", "memory-initial.json");
