...
- 提交 SHA：`f8432e83e40331bca9ccf2e74898b9f7b0fb39b8`
- 偏差：用户要求每项新增回归都先在当前旧代码上失败；本项新增配置矩阵首次即绿，严格红灯前置未能复现。按“不得削弱/人为破坏测试、不得扩大架构”约束，保留该事实并只提交测试扩展。

### P11-01 架构文档

- 状态：`completed`
- 改动文件：`docs/architecture/tool-invocation-path-invariance.md`、`tests/tool-invocation-boundary.test.ts`、`TOOL_INVOCATION_REPAIR_PROGRESS.md`。
- 红灯命令：`set -o pipefail; npx vitest run tests/tool-invocation-boundary.test.ts 2>&1 | tee /tmp/lingxi-tool-contract-p1101-red.log`
- 红灯原始结果：exit `1`；`1 failed` file、`1 failed / 3 passed` tests；任务书指定架构文档不存在，读取返回 `ENOENT`，符合预期。
...
- 最后一次源码候选：`6cf329ff556158f9c3740432f0b722e070172632`。该坐标已推送且本地/远端一致，但因 P12-02 全量测试仍非零，不得升级为“已验证源码候选”，不得进入审计封印。
- 本项内可修复红灯：首轮全量测试还发现持久化指纹失配 `4` 项和媒体观测夹具遗漏规范目标 `2` 项。按当前项范围完成兼容指纹复核与夹具修复，相关门禁 `31/31` 通过；提交 `41384e1aa05a0ee1ce14b89e872363c1cfe45cb4`。随后移除自己新增的 1 条 ESLint 警告，提交 `6cf329ff556158f9c3740432f0b722e070172632`。两次均已推送。
- 最后一次 P12-01：raw execution 边界扫描 exit `0`，扫描 `2121` 个生产源码文件、`0` 违规；指定矩阵 exit `0`，`25 passed` files、`387 passed` tests、无新增 skip。
- 最后一次 P12-02 静态门禁：`npm run typecheck` exit `0`；`npm run lint` exit `0`，`0 errors / 9225 warnings`；`npm run lint:boundary` exit `0`，仅 `1` 条既有债务；`git diff --check` exit `0`。
- 最后一次全量测试：exit `1`；`2 failed / 1345 passed / 1 skipped` files，`2 failed / 13572 passed / 7 skipped` tests。相对固定基线，skip 仍为 `7`，没有增加；本任务相关的指纹和媒体观测失败均已消失。
- 稳定阻塞码一：`P12_SEQUENCE_SEAL_GATE_CYCLE`。全量测试中的审计封印门禁读取旧坐标 `56dc1086aa06d0592c12e8a53eef9b8ea8546812`，因此把本任务源码判为非审计改动；但任务书要求只有 P12-02 全量测试全部 exit `0` 后，才能在 P12-04 更新该坐标。提前更新会跳步并把未完成的构建验证冒充已封印，故未执行。
- 稳定阻塞码二：`RELEASE_PREFLIGHT_REMOTE_VERSION_AHEAD`。当前 `package.json` 为 `0.1.33`，本地和 GitHub 远端 `v0.1.34` 都解析到 `8c2e80e7e00b993a260a3e9273a85be1678c3b94`，发布预检因此稳定报 `candidate 0.1.33 is not greater than historical maximum 0.1.34`。版本升级不在本任务白名单内，未改版本、未删标签、未伪造离线环境。
- 原始日志：`/tmp/lingxi-tool-contract-p1201-boundary.log`、`/tmp/lingxi-tool-contract-p1201-targeted.log`、`/tmp/lingxi-tool-contract-p1202-typecheck.log`、`/tmp/lingxi-tool-contract-p1202-lint.log`、`/tmp/lingxi-tool-contract-p1202-open-boundary.log`、`/tmp/lingxi-tool-contract-p1202-full-tests.log`、`/tmp/lingxi-tool-contract-p1202-persistence-repin.log`、`/tmp/lingxi-tool-contract-p1202-in-scope-repair-gate.log`。
- 后续状态：按用户规则在 P12-02 停止；P12-03、P12-04、P12-05、P12-06 均为 `NOT_EXECUTED_BLOCKED_BY_P12_02`。没有构建、没有 audit seal、没有合并 main。
- 解除条件：由仓库所有者单独授权并完成版本元数据同步，同时明确解决“P12-02 先要求封印测试通过、P12-04 才允许推进封印”的顺序冲突；解除后应以新的源码候选从 P12-01 全部重跑。
...
| 2026-09-05 18:59 +0800 | P12-02 全量测试 | 首轮 `npm test` exit `1`：`8 failed / 13566 passed / 7 skipped`，含 6 个本项可修复失败和 2 个既有门禁失败 | 1 | 兼容 repin 持久化指纹、补齐两条媒体观测夹具的规范目标；相关 `31/31` 通过后形成新候选并从 P12-01 重跑 |
| 2026-09-05 19:07 +0800 | P12-02 外部/顺序阻塞 | 最终 `npm test` exit `1`：仅剩旧审计坐标与远端 `v0.1.34` 两项；`2 failed / 13572 passed / 7 skipped` | 1 | 记录 `P12_SEQUENCE_SEAL_GATE_CYCLE` 与 `RELEASE_PREFLIGHT_REMOTE_VERSION_AHEAD`；不跳步封印、不改版本、不删标签，停止 P12-03 以后步骤 |

## 断点续跑自检

| 问题 | 答案 |
| --- | --- |
| 现在在哪里？ | P12-02 最终复跑只剩审计封印顺序循环和远端版本领先两项稳定阻塞，后续已停止 |
| 接下来去哪？ | 等待仓库所有者同步版本元数据并明确封印顺序；解除后从新的源码候选重新执行 P12-01 至 P12-06 |
| 最终目标是什么？ | 证明并修复工具调用语义对执行路径不敏感，完成 P0–P12 全部门禁与审计封印 |
| 已学到什么？ | 12 个 bundled 工具均为 legacy 权限方言；7 个只读、5 个副作用；当前包装不保留延迟元数据 |
| 已做什么？ | 完成并推送 P0-00 至 P11-02；P12-01 全绿；P12-02 的任务内回归已清零，保留 2 条范围外失败并按规则停止 |
