 ...
 * Ollama 用原生 embed 端点、Gemini 用原生 batchEmbedContents、MiniMax 用官方自有
 * embeddings 协议（texts/type + GroupId query 参数）；重排侧 DashScope 走专属双端点
 * 方言（gte-rerank 系与 qwen3-vl-rerank 系走原生嵌套协议、qwen3-rerank 系走兼容端点），
 * 其余（SiliconFlow/Together/千帆/方舟/智谱/Fireworks 等 OpenAI 兼容网关）的
 * /rerank 均为 cohere 形状、嵌入统一 openai 形状。
...
export function inferOperationProtocol(providerId: string, operation: ModelOperation): ModelOperationProtocol {
  if (operation === "rerank") {
    return providerId === "dashscope" ? "dashscope-rerank" : "cohere-rerank";
  }
  switch (providerId) {
    case "ollama":
      return "ollama-embed";
 ...
 * 2026-08-30 由 100 收紧到 50：火山方舟 doubao-rerank 单次文档上限 50（官方页需登录，
 * 社区实测信源，保守取齐），且 rerank 精度在远小于该值时已饱和，收紧无精度损失。
 */
export const MODEL_OPERATION_RERANK_MAX_DOCS = 50;
