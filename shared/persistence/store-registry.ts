...
export const PERSISTENT_STORES: readonly StoreDescriptor[] = Object.freeze([
  ...
  defineStore({
    ...
    schemaSource: directorySource(
      ...
      "Regenerable lexical SQLite schema together with the vector cache schema in lib/knowledge/vector-index-adapter.ts; both derive only from durable Knowledge facts and artifacts",
    ),
    openEntry: ["new KnowledgeManager", "new KnowledgeIndexStore", "new PortableVectorIndexAdapter"],
    migrationEntry: ["KnowledgeIndexStore runtime DDL", "PortableVectorIndexAdapter runtime DDL"],
    protocolModules: ["lib/knowledge/vector-index-adapter.ts"],
    epochPolicy: "regenerable",
  ...
  defineStore({
    id: "local-model-artifacts",
    ownerModule: "lib/local-models/subsystem.ts",
    pathPatterns: [
      "models/embedding/{idAtQuant}/**",
      "models/ocr/{idAtQuant}/**",
      "models/stt/{idAtQuant}/**",
      "models/tts/{idAtQuant}/**",
      "models/.install-work/**",
      "runtime/local-inference/{runtimeId}/{version}/{platform}/**",
    ],
    pathKind: "tree",
    format: "mixed-directory",
    schemaSource: directorySource(
      "lib/local-models/registry.ts",
      "strict model.json plus per-file size/SHA-256 contract; installer.ts owns runtime.json and atomic staging",
    ),
    openEntry: ["LocalModelsSubsystem.initialize", "LocalModelInstaller.install", "LocalModelRegistry.importDirectory"],
    protocolModules: [
      "lib/local-models/registry.ts",
      "lib/local-models/installer.ts",
      "lib/local-models/secure-archive.ts",
    ],
    firstPossibleOpenPhase: "runtime_ready",
    firstPossibleWritePhase: "runtime_ready",
    epochPolicy: "compatible",
    checkpointPolicy: "Excluded from automatic data-epoch checkpoints because model/runtime assets can be many gigabytes and are independently integrity-addressed.",
    restorePolicy: "Remote assets are redownloaded and reverified from a valid manifest; manual assets are restored only by explicit user re-import or external backup.",
    affectedByEpochMigration: false,
    identityContract: "category/id@quant identifies one model variant generation via model.json version; runtimeId/version/platform identifies one verified runtime generation; .staging-* and .install-work are unpublished transaction state.",
    siteRules: [
      ...rules(
        ["lib/local-models/registry.ts"],
        "Scans, atomically imports, verifies, or removes one exact local model variant directory.",
        ["mkdir", "write-file", "rename", "remove-path"],
      ),
      ...rules(
        ["lib/local-models/installer.ts"],
        "Stages and atomically publishes manifest-verified model and platform runtime assets.",
        ["mkdir", "write-file", "rename", "remove-path"],
      ),
      ...rules(
        ["lib/local-models/backend-probe.ts"],
        "Writes the backend probe cache (probe-cache.json) under LINGXI_HOME runtime root; "
        + "a per-runtime startup捷径 cache that is rebuilt from live probing on any mismatch.",
        ["mkdir", "write-file"],
      ),
      ...rules(
        ["lib/local-models/secure-archive.ts"],
        "Streams declared archive entries only into an installer-owned staging directory and removes it on failure.",
        ["mkdir", "write-file", "remove-path"],
      ),
      ...rules(
        ["desktop/main.cjs"],
        "Creates the fixed local-model root before opening it in the system file manager.",
        ["mkdir"],
        "modelsPath",
      ),
    ],
  ...
  defineStore({
    id: "local-model-downloads",
    ownerModule: "lib/local-models/downloader.ts",
    pathPatterns: ["models/.downloads/{taskId}/**"],
    pathKind: "tree",
    format: "mixed-directory",
    schemaSource: runtimeSource("lib/local-models/downloader.ts", "schemaVersion 1 state.json, deterministic part layout, and verified artifact.bin lifecycle"),
    openEntry: ["ResumableDownloader.download", "ResumableDownloader.listTasks"],
    firstPossibleOpenPhase: "runtime_ready",
    firstPossibleWritePhase: "runtime_ready",
    epochPolicy: "regenerable",
    checkpointPolicy: "Excluded; incomplete parts can resume from the source asset and completed artifacts are deleted after publication.",
    restorePolicy: "Resume only when URI, byte count, SHA-256, and deterministic taskId still match; otherwise cancel and redownload.",
    affectedByEpochMigration: false,
    identityContract: "taskId is SHA-256(uri, bytes, expected SHA-256) and owns exactly one state file, part set, and optional verified artifact.",
    siteRules: rules(
      ["lib/local-models/downloader.ts"],
      "Persists resumable download state, bounded parts, assembly output, and exact-task cleanup.",
      ["mkdir", "write-file", "rename", "remove-path"],
    ),
  }),
  defineStore({
    id: "local-model-manifest-cache",
    ownerModule: "lib/local-models/manifest-client.ts",
    pathPatterns: ["models/.manifest-cache/manifest-cache.json"],
    format: "json",
    schemaSource: runtimeSource("lib/local-models/manifest-client.ts", "schemaVersion 1 URL/ETag/fetchedAt/raw-body parser with strict manifest revalidation"),
    openEntry: ["LocalModelsManifestClient.loadCached", "LocalModelsManifestClient.refresh"],
    firstPossibleOpenPhase: "runtime_ready",
    firstPossibleWritePhase: "runtime_ready",
    epochPolicy: "regenerable",
    checkpointPolicy: "Excluded; it is an HTTPS/ETag cache and never the authority for installed bytes.",
    restorePolicy: "Re-parse before use; discard invalid cache and refresh, falling back to the built-in catalog while offline.",
    affectedByEpochMigration: false,
    identityContract: "One cache record belongs to one configured manifest URL for one LINGXI_HOME.",
    siteRules: rules(
      ["lib/local-models/manifest-client.ts"],
      "Creates the cache directory and atomically replaces the last strictly parsed manifest response.",
      ["mkdir", "write-file", "rename"],
    ),
  }),
  defineStore({
    id: "local-model-sidecar-logs",
    ownerModule: "lib/local-models/sidecar-manager.ts",
    pathPatterns: ["logs/local-models/{idAtQuant}/sidecar.log*"],
    pathKind: "tree",
    format: "append-only-log",
    schemaSource: directorySource("lib/local-models/sidecar-manager.ts", "redacted bounded rotating stderr log family"),
    openEntry: ["SidecarManager.start"],
    firstPossibleOpenPhase: "runtime_ready",
    firstPossibleWritePhase: "runtime_ready",
    epochPolicy: "regenerable",
    checkpointPolicy: "Excluded; logs are bounded diagnostics and contain no model output payload by host design.",
    restorePolicy: "Never restore; create a new bounded log generation after restart.",
    affectedByEpochMigration: false,
    identityContract: "id@quant owns one active redacted log and at most maxLogFiles rotated generations.",
    siteRules: rules(
      ["lib/local-models/sidecar-manager.ts"],
      "Creates sidecar log directories and appends/rotates token-redacted stderr diagnostics.",
      ["mkdir", "append-file", "rename", "remove-path"],
    ),
  }),
]);
