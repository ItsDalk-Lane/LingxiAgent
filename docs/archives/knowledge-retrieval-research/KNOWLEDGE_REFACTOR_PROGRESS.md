...
> 历史任务台账：执行分支、阶段纪律与提交批准仅适用于当时 P0–P3 任务。当前普通聊天见[知识库执行链](../../architecture/knowledge.md)，后续封印见 [PROGRESS](../../../PROGRESS.md) 对应日期记录。
...
- 当前断点：P0 至 P3-07 实现与门禁全部完成，最后实现源码 `61ae60d1`；本机全量 13751 PASS / 0 FAIL / 7 既有 SKIP，78.81s，第五轮四平台与统一产物门禁全部通过。最终独立封印坐标与复验记录见 PROGRESS.md。
...
| P3 | completed | 全部实现与门禁通过；本机 13751 PASS / 0 FAIL / 7 既有 SKIP；第五轮四平台各 151 PASS，质量 13741 PASS / 0 FAIL / 17 既有 SKIP，统一产物门禁 305 PASS |
...
- 对应 commit SHA：`9f430236`
- 偏差：none

## P3-03：实现 Completeness Executor

- 状态：completed
- 改动文件：新增完整性执行器、分片规划与两项专属工具；真实 Root/专属 Worker 创建、权限、共享预算、结束判断、最终上下文接线；删除/回收引用预检；执行器、分片、不可用来源、否定纪律、工具、真实会话、安全信封与生命周期回归；开放清单、指纹与运行闭包。
- 改动：从冻结解析原文的既有 CoverageUnit 建立分母，全文核查保留全部原文单元；没有可用解析产物的来源单独计入不可用项并明示，不能伪造单元外键或悄悄移出分母。相关章节以 Agent 实际成功搜索所指定的来源/章节键为范围，父章包含其下节；空或未知范围保持部分完成。升级政策或新增相关章节立即使旧证明失效，追加核查后才重新成立。
- 执行：分片按包含位置、凭据、安全信封和原文的实际渲染量控制在 12000 tokens，所有单元保序且不丢失；最多四个真实隔离 Worker，共用 32 次工具/180 秒总预算和每轮 12 次阅读。专属 Worker 只有读取分片与标注两工具，逐次校验真实祖先、范围、分片和角色。相关原文必须经完整读取凭据和精确引文复核，批量标注与证据写入同事务；可读单元不能被任意标为不可用。旧覆盖表只保存身份/hash/不可用原因，不存模型思考或原文副本。取消、异常和写库失败都等待实际工作会话清理后释放额度；尚在运行、失败分片、缺项或不可用项均不得出 exact 证明。最终回答材料按真实完整性限制否定措辞，章节标题保留原文并置于安全信封中。
- 生命周期：研究凭据、证据、完整性单元和研究冻结来源均阻止删除/回收；先预检再改变源状态、监听或任务，旧无引用来源仍能正常删除。
- 测试命令：任务书四项测试，加全部完整性/研究回归、真实详细入口/快速零远程、工具全集、范围祖先、生命周期、证据清单、Engine 持久化和指纹，共 37 文件；三套类型检查；全部 29 项改动代码 ESLint；Node 24 直接导入执行器/Agent/编排器；持久化清单/指纹、闭包、边界和开放树生成；差异检查。
- 测试结果：最终 **37 文件 / 492 PASS / 0 FAIL / 0 SKIP，7.92s，exit 0**；三套类型、ESLint（0 错误/631 警告）、Node 源码运行、生成器与差异检查均 exit 0。真实 Root→专属 Worker→原文读取→标注→清理→最终材料链覆盖完整、漏检、不可用来源三种结果；Worker 末次标注尚未清理时仍为非 exact。取消及真实 SQLite 收尾故障保持等待全部 Worker；32 次/180 秒超限不能新开核查。日志 `/tmp/lingxi-knowledge-p303-tests-reviewed.log`、`/tmp/lingxi-knowledge-p303-typecheck-final.log`、`/tmp/lingxi-knowledge-p303-lint-final.log`、`/tmp/lingxi-knowledge-p303-node-source.log`。
- 失败与修复：首次接线 57 PASS/2 FAIL，一处测试误把已删除会话墓碑当作不存在，改为同时验证墓碑和文件已删除；一处真实失败计数把已检查单元重复计入失败，保留原计数断言修复生产逻辑。新范围使旧证明失效的两项红测修复后通过。章节标题原先直拼可关闭材料边界，只读复现后修为保留原文的安全信封并新增真实完整性渲染回归。相关日志 `/tmp/lingxi-knowledge-p303-wiring-first.log`、`/tmp/lingxi-knowledge-p303-proof-freshness-red.log`、`/tmp/lingxi-knowledge-p303-section-boundary-review.log`；全部原断言保留，无新增跳过。
- 最后复核：核查完成与完整证明原先误共用 exact 判断。按任务书保留所有计数/禁止完整否定断言，将含不可用项但已全部处理的核查状态校正为 completed、exact=false，并增加真实落库状态断言；先 3 PASS/2 FAIL，再修复生产状态分支，完整联测 492 PASS。研究完成条件仍要求完整性策略满足，不把该核查状态当作完整证明。日志 `/tmp/lingxi-knowledge-p303-completion-state-red.log`；修改两文件 ESLint 和闭包复验通过。
- 生成物：持久化清单 66 stores / 779 sites；知识库 v19 不再增加字段，兼容指纹 `sha256:5ea5991e78a826a99f33de0c44e770b14b0813bf95be73fe648d7248196fe079`；运行闭包 10693 文件，开放树 887 文件。兼容指纹由生成器反映既有表核查和专属会话身份守卫。P3 阶段门禁与最终封印仍待剩余任务完成。
- 对应 commit SHA：`350c0588`
- 偏差：none

## P3-04：建立 source / section / span 多粒度索引

- 状态：completed
- 改动文件：分块、FTS 索引、摄入、管理器、查询与冻结范围编译、搜索及两种证据清单接线；持久化登记；五项指定新测试、混合格式回归及旧版本测试适配。
- 改动：chunker v3 固定 512 tokens 原文片段、64 tokens 相邻重叠、8192 tokens 章节上限；新默认配置身份使用固定 2048 字符标度，显式旧配置仅保留身份区分，不能把片段放大。模型窗口只在实际嵌入前检查硬上限，不能据此改变检索粒度。Markdown/HTML按标题、章节文本按章标题、PDF优先标题否则页组分节；巨块按原文连续区间拆子节，同父标题不变，每原块唯一 primary 归属，所有章节区间无重叠、无遗漏，最终片段二级映射回原始块。旧 v2 算法保留显式历史入口，旧配置/索引/向量不覆盖。
- 索引：FTS v3→v4 仅新增锁定来源/章节表与各自 FTS5，片段追加 section_id 和两索引；来源标题、章节提纲、首末片段和文件类型均确定性生成，零模型调用。三层写入同事务，未知章节/跨产物归属拒绝。真实 v3 fixture 由 `350c0588` 原公开索引 API 生成，SHA256 `8b8f99fb75ed1211f0ebabb59b0370bfffd76ef50a64c1dfa04909f4bac482b9`；迁移中段 DDL 故障整体回滚且保留旧库，只有真实 SQLite 损坏才重建缓存。
- 后台：启动返回后每批 20 来源扫描活跃最新资料，每来源仅一份低优先级候选，普通排队/在途任务优先；不新增队列表字段。旧排队/待嵌入任务保留同 job 身份先补新分块，运行中任务与旧向量不改。查询编译只读元数据，新变体未就绪时继续查旧变体、明确留痕且异步请求补建；两者都无则明确降级。实际完成的新向量沿既有规则生成 HNSW；后台等待嵌入时有空闲名额仍及时处理新普通任务。
- 验证中已通过：旧知识全量首轮 102 文件 / 1034 PASS / 18 FAIL / 0 SKIP，20.69s；失败逐项修正后第二轮 **103 文件 / 1057 PASS / 0 FAIL / 0 SKIP，20.85s**。专项分块新旧 37 PASS、索引与迁移 36 PASS、后台及摄入 25 PASS、实际混用与配置 18 PASS；三套类型与改动代码 ESLint 已通过。首轮完整日志 `/tmp/lingxi-knowledge-p304-allknowledge-first.log`，第二轮 `/tmp/lingxi-knowledge-p304-allknowledge-second.log`，混合格式复核与生成物同步结果见下。
- 原断言保留情况：旧纯分块/索引测试机械切显式 v2 入口；新生产默认版本/固定片段数按任务书更新并增加 512 上限和原文映射断言。大候选重排 fixture 增加实际素材保留 60/50/尾10断言；句界正例改成完整目标句落在单片段内，同时保留原长素材为跨片段精确引用新用例；异步入队先断言返回时无任务、下一轮事件循环仍恰一任务。原约束、来源数量、引用、预算和历史数据不改，没有新增跳过。
...
- 状态：completed
- 改动文件：八类完整性 JSON 资料、研究质量夹具和测试、真实最终会话/HTTP/观测合成夹具；快速/向量/研究三项基准、三份测量生成物与性能 CI。
- 质量：全文人名、术语从未出现、每章变化、全部反例、needs OCR、解析失败、跨章重复事实、同源冲突均经真实 Engine、Agent 工具、冻结原文、完整性执行器、凭据和最终清单。另保留 P2 第二轮线索、跨来源、时间线等八项质量验证。逐项报告任务书十个指标，不适用冲突指标为 null，不伪造为通过率。最终清单逐跨度复核已消费凭据/hash/原文和账本，真正持久化后再次核对。不可用来源保留分母，遗漏末单元也拒绝完整否定；重复事实保留不同章节位置，冲突保持 conflicted。
- 观测与边界：研究模型决策为固定脚本，所有动作经过真实工具/台账，逐次记录轮次、工作会话、搜索与阅读耗时；最终合成通过真实 AgentSession→HTTP→本地固定响应供应商→流式返回，四层调用记录均持久化，并复核最终引用与供应商报告用量。供应商替身报告的 token 数由夹具估算，不是付费模型实测或账单；最终合成耗时也是本地传输/观测链耗时。报告明确区分该边界，不将模型替身称为在线模型质量或速度证明。
- 最终测试：知识全量加原文清单、持久化指纹、工具权限、桌面提交与真实模型观测会话，**117 文件 / 1194 PASS / 0 FAIL / 0 SKIP / 0 errors，23.56s，exit 0**；三套类型、8 个改动代码文件 ESLint、差异检查均 exit 0，均已等待进程实际结束。日志 `/tmp/lingxi-knowledge-p306-tests-reviewed.log`、`/tmp/lingxi-knowledge-p306-typecheck-final.log`、`/tmp/lingxi-knowledge-p306-lint-reviewed.log`。
- 失败与修复：新夹具首次错误地在研究结束后重新写需求状态（9 FAIL/8 PASS），改用只读评估，保留生产终态禁止写入；修后新旧质量 17 PASS。快速基准首轮元数据 API 参数形状错误，改为实际产物/profile 对象；向量基准局部变量拼错由 ESLint 查出并修正。全联测首轮 1193 PASS/1 FAIL，仅旧基准 schema/seed 标识断言未跟随真实三层索引版本，更新标识且保留全部性能/数量/零远程断言，最终 1194 PASS。原日志保留，没有跳测或放宽门禁。
- 基准：本机 Node 24.16/macOS arm64，实际硬件信息随 JSON 保存。快速 10k 热 P95 **3.617ms**/冷 **49.360ms**；100k 热 **39.497ms**/冷 **481.581ms**，每组 20 热样本/5 冷样本、15 次热缓存命中、远程 0。真实 v3 摄入创建来源/章节/片段，规模断言仍精确 10k/100k。100k 向量 HNSW P95 **3.963ms**、portable **157.925ms**、比值 **39.85x**、top10 重合 **99.75%**，64 片段章节补查 P95 **0.404ms**。稳定墙钟门禁启用且通过。研究八资料首动作 **0.791～1.815ms**，1 或 3 轮、6 或 8 工具调用、最终引用 100%；可用资料 coverage=1，不可用样本 2/3、不可用计数 1。最终本地合成 **3.589～39.027ms**，每次 1 个真实 HTTP 调用与 1 个持久化调用记录。
- 生成物：`artifacts/knowledge-{fast,vector,research}-benchmark-p306-darwin-arm64.json`，由脚本生成；日志 `/tmp/lingxi-knowledge-p306-{fast-benchmark-fixed,vector-benchmark,research-benchmark-final}.log`。五生成器 exit 0，66 stores/779 sites、原指纹 `sha256:a385729c34ac35cda0d6eafbfe98061dc2eb6c88519e409179f5f00005e42569`、运行闭包 10693/开放树 887，不改 schema。性能 CI 增加构建工作区依赖和研究基准上传，远程执行留到 P3 最终源代码验证。
- 对应 commit SHA：22b46292
- 偏差：none；测量边界见上，不冒称在线供应商实测。

## P3-07：清理旧架构和最终收口

- 状态：completed
- 改动：删除引擎旧详细入口、重复查询和范围解析、旧固定候选池及快速重排门控。原旧算法移至 `tests/fixtures/knowledge-legacy/`，原回归继续执行；生产清单门面仍由真实快速/研究流程共同使用。旧覆盖计划模块只留历史读取类型和校验。正式入口测试走真实研究工具、阅读凭据与台账，增加生产入口缺席断言。
- 验证：知识全套 110 文件 / 1087 PASS / 0 FAIL / 0 SKIP，22.69s；三组类型检查 exit 0；本机平台 19 文件 / 151 PASS / 0 FAIL / 0 SKIP，25.86s；打包结构与真实原生移除测试 4 文件 / 30 PASS，2.53s。全量 lint exit 0（9190 warnings）；boundary exit 0。
- 初轮全量：1354 文件 PASS / 2 FAIL / 1 既有 SKIP，13734 测试 PASS / 2 FAIL / 7 既有 SKIP，90.72s，exit 1。失败：旧审计坐标尚未推进；两项完整性内部工具未按既有隔离工具规则登记。后者补登记并新增真实工具边界断言，不删除测试或改变判断标准；封印待最终源提交后按用户授权同步。
- 初轮生成：66 stores / 779 sites；指纹保持 `sha256:a385729c34ac35cda0d6eafbfe98061dc2eb6c88519e409179f5f00005e42569`；闭包 10691 文件，开放树 885 文件，均 exit 0。
- 实际服务端、开放服务端、客户端五入口、种子验签与归档烟测全部 exit 0；重启后真实本地检索命中原文，移走原生扩展后全新进程确认 portable 和本地检索均通过。工具登记修复及隔离权限复验 52 PASS，接线复核 36 PASS。日志 `/tmp/lingxi-knowledge-p307-{build-server,verify-seed,packaged-smoke,build-open,build-client,label-fixed,wiring-reviewed}.log`。
- 完整应用打包、最终四平台、生成器二轮和最终报告仍进行中；尚未通过的项未标 PASS。
- 对应 commit SHA：`c860054b9ee7f961abbb0cfebc2bbd9428ab6bd3`
- 偏差：none

- P3-07 本地首次完整打包 exit 1，145.034s：临时运行环境将已有 `SKIP_NOTARIZE` 错写为 `1`（脚本仅接受 `true`），本地 ad-hoc 签名完成后误入需要 Apple 密码的公证。只纠正临时环境开关并完整重跑，不改打包脚本、测试或正式签名配置；原始日志 `/tmp/lingxi-knowledge-p307-pack.log` 保留。

- P3-07 完整 `npm run pack` 纠正后 exit 0，86.150s，本地 ad-hoc、未公证。最终类型检查 26.404s / lint 17.497s / boundary 0.897s 全部 exit 0。准备固定实现源码提交，之后复验完整生成物与审计门禁，再执行当前源码四平台；最终封印未建立。

- P3-07 补齐桌面启动证据：本机真实打包桌面先验证应用就绪通知/服务端状态，随后增强为本地调试协议读取真实页面状态（仅读取，不修改页面），首启引导页 complete、93 字符、7 个交互控件，服务端知识接口 200；8.691s PASS，进程和随机资料目录均清理。四平台 Build 增加对应桌面启动步骤，Linux 用虚拟显示；接线测试 15 PASS，200ms。新增脚本静态检查曾因全局 WebSocket 名未登记报 1 error，改为显式 globalThis 引用后复验。
- 前一版未推送实现提交的完整审计复验：13737 PASS / 0 FAIL / 7 既有 SKIP，85.97s。为把桌面启动检查纳入同一个 P3-07 实现提交，在推送前修订该提交；之后重新核验生成器与审计，旧日志保留，不冒充最终版本结果。

- 最终实现 `c860054b`：全量 13738 PASS / 0 FAIL / 7 既有 SKIP，87.02s；三套类型、lint、边界全部 exit 0。两轮生成器共 14 条命令全部通过、全工作区差异不变，960 项测试清单和 885 个开放树文件逐字节一致。阶段审计 `e296c017` 已提交并推送；四平台 Build `33850016811` 执行中，最终独立封印尚未建立。

- 最终源码构建时间补录：正式服务端 111.740s、种子验签 3.725s、包内重启/移除原生扩展后的真实检索 11.209s、开放服务端 38.797s、客户端五入口 10.473s，均 exit 0。最终源码三项性能基准也均通过，原始数据和命令时间已加入交付产物。

- 当前 P3 四平台首轮 Linux：知识专项与性能、归档重启检索及完整打包通过，真实桌面启动 exit 1。解包目录 chrome-sandbox 没有系统安装后的 root:root / 4755 权限，Chromium 主动拒绝启动。测试环境增加正确属主和权限准备并检查实际值，仍启用沙箱、原真实页面与后台断言不变；接线测试 15 PASS，160ms，定向 ESLint exit 0。原始失败保留 `artifacts/knowledge-desktop-startup-e296c017-linux-x64-failed.json`；修复后需远程重验，不能仅据静态测试宣布通过。

- macOS arm64 与 Windows 的首次桌面启动也失败，原有报告仅保留概括错误，无法确定具体失败事件。补充最多 30 条脱敏启动事件及有界进程日志，成功条件不变；相关接线与脱敏测试 22 PASS，159ms，ESLint exit 0。使用 CI 实际共享界面归档在本机完整重打包 exit 0，真实桌面引导页及知识接口成功（10.825s），未复现远程故障；原远程失败 JSON 保留，需通过新一轮带诊断的真实平台运行继续定位。

- 启动修复后的首次本机全量出现下载清理竞态：13737 PASS / 1 FAIL / 7 既有 SKIP，83.95s。超限响应到达时文件仍在异步打开，错误清理先删文件，随后打开又创建残留。新增延迟打开的确定性红测证实（单测 1 FAIL，其余按名称过滤未执行），改为销毁写入端并等待 close 后删除，不改变下载上限。原测试保留；整文件 106 PASS，1.09s；三组类型 exit 0，测试 ESLint 0 errors，运行源文件 Node 语法检查通过。生成器指纹保持不变；测试清单由生成器更新。原全量和红测日志保留。
- 四平台首轮 Build `33850016811` 最终 failure：质量门禁 13728 PASS / 0 FAIL / 17 平台既有 SKIP，601.72s；四平台知识专项、原生/精确回退、归档重启检索和完整打包都通过，四平台桌面启动失败；由此下游 artifact-release-smoke 未执行，不能记为通过。原始逐步骤结果和四份启动失败 JSON 已保存。
...
- 进程树清理修复 `93764185`：本机真实打包启动与清理通过（8.647s）；全量 13746 PASS / 0 FAIL / 7 既有 SKIP，83.65s；两轮生成器及 lint 通过。阶段审计 `84247718` 已推送；第四轮 Build `33858404258` 已提交执行。

- 第四轮 Windows 实机启动与清理已通过：打包进程、引导页及后台就绪，启动 65.529s，`cleanupPassed=true`。Linux、macOS arm64 和远程全量门禁也已完整通过；macOS Intel 及后续产物门禁继续执行，不提前将整轮标为通过。

- 第四轮第 1 次尝试仅 Intel DMG 因临时磁盘 `Resource busy` 未能卸载而失败，启动未执行；Windows 清理修复和其他平台均已通过。环境状态已写 BLOCKED.md，保留完整失败记录，按同一提交重新执行失败任务及后续产物门禁，不改打包命令、测试或阈值。

- 第四轮第 2 次尝试：Intel DMG 通过，环境临时磁盘阻塞解除；桌面在 4.279s 因首次调试 HTTP 探测 2s 超时提前退出，原 90s 总期限尚未达到。修正探测就绪语义：仅单次 TimeoutError 记计数并返回未就绪，继续受原总期限控制；非法响应、真实崩溃、页面加载失败以及最终页面/后台条件不变。确定性红测 1 FAIL/8 PASS（134ms），修后关联 3 文件 31 PASS（142ms），未删除或放宽原测试。

- 就绪探测修复 `61ae60d1` 全量 13751 PASS / 0 FAIL / 7 既有 SKIP（78.81s），两轮生成器及 lint 通过，阶段审计 `790b496d` 已推送；第五轮 Build `33864141539` 执行中。

- 最终第五轮 Build `33864141539`（实现 `61ae60d1`，阶段审计 `790b496d`）全部成功：四平台知识专项各 151 PASS、包内迁移/原生和精确回退/重启检索、完整打包、真实桌面与清理全部通过；远程质量 13741 PASS / 0 FAIL / 17 既有 SKIP，643.87s；统一产物门禁 305 PASS / 0 FAIL / 0 SKIP，1.63s。原始结果和最终报告已完整归档。后续纯审计封印的验证坐标与结果以 PROGRESS.md 为准。

## 2026-09-04 用户反馈：详细模式会话组装失败

- 已确认 `faad0da24` 的研究扩展覆盖漏掉 SDK 必需的运行载体，真实创建会话时抛出 `Cannot set properties of undefined (setting 'sendMessage')`，随后转为界面的详细调查失败。
- 旧装配参数测试在错误代码上仍 18/18 通过，因为替换了整个 SDK 会话创建过程。新增真实生产组装回归在修复前是三个研究入口失败、普通隔离会话通过；修复后五项全部通过，覆盖独立/稳定载体、空扩展/技能、只读工具名单与临时会话清理。
- 修复只补公开工厂创建的每会话独立载体及返回类型约束。相关 119 项、全量 13756 项通过（0 FAIL / 7 既有 SKIP，82.74s），三套类型、lint、持久化与开放边界门禁通过。
- 本机客户端/服务端构建、临时签名种子验签和 macOS arm64 目录包完成；正式公证未配置，Electron 下载连接失败后采用本机同版本 42.8.1。原始失败和最终结果保留于 `artifacts/knowledge-research-runtime-fix/progress.md`，本次不复用旧四平台结果作为新修复发布证明。
- 新修复及独立审计封印坐标以 `PROGRESS.md` 为准；原 P0–P3 交付与封印历史保留，未合并 main。

## 2026-09-05 正式发布 0.1.34 前的契约同步与收尾修复

- 用户后续明确授权重新分析并调整最初方案：普通问答改为当前聊天模型连续查阅，旧快速/详细设置归一；独立研究能力保留专用入口。此次又明确授权合并 main 并发布 0.1.34，先前调试阶段“不测试、不构建、不提交”的限制不再用于阻断正式发布所需验证。
- 分支 `31afdea0` 已包含上述交互、分块与引用变更，但旧回归仍按预检索/独立研究路由、固定 512 词元切分和旧工具返回形状检查。发布前首次全量 157 FAIL；原始失败保留 `/tmp/lingxi-vitest.json`，不作通过记录。
- 逐项同步真实契约：实际分块设置生效且保持章节、原文偏移和旧索引；普通查阅使用冻结原文及引用链接、按消息体积分页；旧研究仍检查凭据伪造、范围越权、共享额度及完整性限制。只对有效条目入库的批次部分成功与全部无效回滚分别检查。未删除用例、跳过失败或放宽权限、证据真实性及预算门禁。
- 发现并修复两处真实收尾缺陷：关闭知识管理器时立即使查询缓存失效，然后等待后台真实退出再关库；停止普通发送后清除会话编号和路径两种停止记录，避免下一次发送误继承停止状态。各测试的关闭清理同步等待异步退出。
- 新增当前聊天准备入口集成回归：同轮范围复用、连续两次实际检索及可引用原文、禁止同轮替换资料/复活已关闭范围、会话身份/工具缺失/预先停止拒绝。新增引用悬浮预览回归：已保存原文、仅文件名、鼠标移入、键盘关闭、失败重试与无障碍关联清理。
- 持久化指纹已按兼容变更重新生成：现有数据表/列及数据代际不变，旧分块和向量仍可读取；差异仅受保护源码摘要。引用样式维持原视觉，颜色与间距按既有样式规则封装为局部变量。
- 本轮代码、生成物、测试及正式发布的最终结果见后续 PROGRESS.md 审计封印和 GitHub 发布记录；旧 P0–P3 及运行载体修复证据保留，不冒充最新源码的跨平台证明。
