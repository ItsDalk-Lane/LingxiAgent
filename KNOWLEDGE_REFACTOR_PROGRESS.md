...
| P3 | in_progress | P3-01 至 P3-04 单项通过；阶段门禁尚未执行 |
...
- P3-07 补齐桌面启动证据：本机真实打包桌面先验证主窗口/服务端就绪，随后增强为本地调试协议读取真实页面状态（仅读取，不修改页面），首启引导页 complete、93 字符、7 个交互控件，服务端知识接口 200；8.691s PASS，进程和随机资料目录均清理。四平台 Build 增加对应桌面启动步骤，Linux 用虚拟显示；接线测试 15 PASS，200ms。新增脚本静态检查曾因全局 WebSocket 名未登记报 1 error，改为显式 globalThis 引用后复验。
