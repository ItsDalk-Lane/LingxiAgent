...
const lingxiFetchMock = lingxiFetch as unknown as ReturnType<typeof vi.fn>;
...
describe('archived-session actions', () => {
  it('listArchivedSessions returns the body array', async () => {
    lingxiFetchMock.mockResolvedValueOnce({
      ok: true,
    ...
    expect(lingxiFetchMock).toHaveBeenCalledWith('/api/sessions/archived');
  });

  it('listArchivedSessions returns [] when fetch not ok', async () => {
    lingxiFetchMock.mockResolvedValueOnce({ ok: false, json: async () => ({ error: 'x' }) });
    const { listArchivedSessions } = await import('../../stores/session-actions');
  ...
  it('deleteArchivedSession posts path/sessionId and returns true on ok', async () => {
    lingxiFetchMock.mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({ ok: true }) });
  ...
  it('deleteArchivedSession returns false when not ok', async () => {
    lingxiFetchMock.mockResolvedValueOnce({ ok: false, status: 403, json: async () => ({ error: 'x' }) });
    const { deleteArchivedSession } = await import('../../stores/session-actions');
