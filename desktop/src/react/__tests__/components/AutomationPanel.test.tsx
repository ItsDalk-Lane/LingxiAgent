...
describe('AutomationPanel', () => {
  ...
  it('shows the structured POST error and keeps the panel usable', async () => {
    ...
    await waitFor(() => expect(lingxiFetch).toHaveBeenCalledWith('/api/desk/cron', {
      throwOnHttpError: false,
    }));
    vi.mocked(lingxiFetch).mockImplementation(async (url, options) => {
    ...
    const postCall = vi.mocked(lingxiFetch).mock.calls.find(([, options]) => options?.method === 'POST');
  ...
  it('shows a structured GET error without presenting an empty task list or clearing the badge', async () => {
    ...
    expect(lingxiFetch).toHaveBeenCalledWith('/api/desk/cron', { throwOnHttpError: false });
  });
});
