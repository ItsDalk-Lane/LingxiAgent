...
const lingxiFetchMock = vi.fn(async (url: string, opts?: RequestInit): Promise<MockResponse> => {
...
describe('Agent memory settings health notice', () => {
  ...
  it('uses an action-specific localized fallback for an uncoded English failure', async () => {
    ...
    expect(screen.queryByText('settings lingxiFetch: server connection not ready')).not.toBeInTheDocument();
  });
