 ...
 *
 * lingxiFetch 默认 throwOnHttpError=true：非 2xx 直接抛错，调用方按失败处理，
 * 不做静默降级。
 */
...
export async function fetchHistoryFiles(agentId: string): Promise<FileHistoryFileEntry[]> {
  const res = await lingxiFetch(`/api/file-history/files?agentId=${encodeURIComponent(agentId)}`);
...
export async function restoreHistorySnapshot(
  ...
  const res = await lingxiFetch('/api/file-history/restore', {
    method: 'POST',
