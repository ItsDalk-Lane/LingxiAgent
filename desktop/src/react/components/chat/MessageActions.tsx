...
export function useMessageFooterActions({
  ...
  return useMemo(() => {
    const feedbackActions: MessageFooterAction[] = typeof onFeedback === 'function'
      ? [
          {
            id: 'feedback-up',
            title: t('chat.feedbackUp'),
            icon: <ThumbUpIcon active={feedbackState === 'up'} />,
          ...
          },
          {
            ...
            icon: <ThumbDownIcon active={feedbackState === 'down'} />,
...
function ThumbUpIcon({ active }: { active: boolean }) {
  return (
    <svg width="15" height="15" viewBox="0 0 24 24" fill={active ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" opacity={active ? 0.9 : undefined}>
      <path d="M7 10v12" />
