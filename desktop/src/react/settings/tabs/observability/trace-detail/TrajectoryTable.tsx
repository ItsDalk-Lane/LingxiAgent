/**
 * TrajectoryTable.tsx — 轮次感知的轨迹事件台账 + 局部记录检查器。
 *
 * 移植自 dsh-desktop packages/client/ui-trajectory/src/client/TrajectoryTable.tsx
 * （MIT）。适配点：
 *   - dsh ui-primitives（JsonTree/MarkdownText/Tooltip/图标）→ 灵犀等价物
 *     （JsonValueViewer / renderMarkdownPreview / ui Tooltip / 内联 SVG）；
 *   - 文案全部走灵犀 i18n（settings.observability.traceDetail.*）；
 *   - purpose 'compaction' → 'side'（轮次间隙侧线调用的语义泛化）；
 *   - 灵犀独有合并：检查器新增「观测载荷」tab（payloads）——按记录的
 *     observabilityCallId 懒加载调用详情与载荷正文（未开捕获时显式不可用态）。
 */
...
import { payloadKindLabel } from '../model-observability-labels';
import { shortId } from '../model-observability-format';
import { payloadToReadableText } from './payload-plain-text';
import { JsonValueViewer } from '../JsonValueViewer';
import type {
  AssistantMetricDetail, TrajectoryCellKind, TrajectoryCellProps, TrajectoryPromptSnapshot,
...
function kindLabelOf(kind: TrajectoryCellKind): string {
  return tr(`kind.${kind}`);
}
...
function ChevronRightIcon({ size = 12, className }: { size?: number; className?: string }): ReactNode {
  return (
    <svg
      className={className}
      width={size}
      ...
      aria-hidden="true"
    >
      <path d="m5 2.5 4.5 4.5L5 11.5" />
    </svg>
  );
}

function KIND_ICON(kind: TrajectoryCellKind): ReactNode {
  switch (kind) {
    case 'system': return <SettingsIcon />;
    case 'user': return <UserIcon />;
    case 'context': return <InformationIcon />;
    case 'compacted': return <CompactedIcon />;
    case 'message': return <SparkleIcon />;
    case 'tool':
...
function formatStartedAt(timestamp: number | null): string {
  if (timestamp === null || !Number.isFinite(timestamp)) return tr('detail.notAvailable');
...
function StartedAtValue({ timestamp }: { timestamp: number | null }) {
  ...
  if (timestamp === null || !Number.isFinite(timestamp)) {
    return <dd>{tr('detail.notAvailable')}</dd>;
  }
  return (
    <dd>
      <button
        ...
        title={showUnix ? tr('detail.showLocalTime') : tr('detail.showUnix')}
        onClick={(event) => {
...
/* ── 灵犀替换件：JsonTree / MarkdownText ──────────────────────────────── */

function JsonTree({ data, className }: { data: object; className?: string }) {
  return (
    <div className={className}>
      <JsonValueViewer value={data} />
    </div>
  );
}

function MarkdownText({ text }: { text: string }) {
  const html = useMemo(() => renderMarkdownPreview(text), [text]);
...
export interface TrajectoryTableProps {
  ...
  /** 灵犀扩展：观测载荷 tab 的本机 owner 判定（远端 403 → 仅本机可看）。 */
...
/** 与 session 全局编号配对的请求身份（purpose 判别）。 */
...
function sectionLabel(turn: number | null): string {
  return turn === null ? tr('table.betweenTurns') : tr('table.turn').replace('{n}', String(turn));
}
...
function summarizeTurn(records: readonly TableRecord[]): string {
  ...
  return [
    `${steps} ${steps === 1 ? tr('table.stepOne') : tr('table.steps')}`,
    `${toolCalls} ${toolCalls === 1 ? tr('table.toolCallOne') : tr('table.toolCalls')}`,
  ].join(' · ');
}
...
function summarizeAssistantTools(records: readonly TableRecord[]): string {
  ...
  const summary = `${count} ${count === 1 ? tr('table.toolCallOne') : tr('table.toolCalls')}`;
...
function statusLabel(state: RecordState): string {
  if (state === 'error') return tr('detail.statusFailed');
  if (state === 'running') return tr('detail.statusPending');
  return tr('detail.statusCompleted');
}
...
function UsageRows({ usage }: { usage: TrajectoryUsage | undefined }) {
  if (usage === undefined) return <p className={css.noPayload}>{tr('detail.usageNotReported')}</p>;
  ...
  return (
    <dl className={css.overview}>
      {totalInput !== undefined && (
        <div><dt>{tr('detail.input')}</dt><dd>{totalInput} tok</dd></div>
      )}
      {usage.cacheRead !== undefined && (
        <div className={css.requestTokenDetail}>
          <dt>{tr('detail.cached')}</dt>
          <dd>{usage.cacheRead} tok</dd>
        </div>
      )}
      {usage.cacheWrite !== undefined && (
        <div className={css.requestTokenDetail}>
          <dt>{tr('detail.cacheCreated')}</dt>
          <dd>{usage.cacheWrite} tok</dd>
        </div>
      )}
      {usage.input !== undefined && (
        <div className={css.requestTokenDetail}>
          <dt>{tr('detail.other')}</dt>
          <dd>{usage.input} tok</dd>
        </div>
      )}
      {usage.output !== undefined && (
        <div><dt>{tr('detail.output')}</dt><dd>{usage.output} tok</dd></div>
      )}
  ...
  return (
    <div className={css.usagePanel}>
      <section className={css.usageGroup}>
        <h4 className={css.usageHeading}>{tr('detail.thisRequest')}</h4>
        <UsageRows usage={usage} />
      </section>
      <section className={css.usageGroup}>
        <h4 className={css.usageHeading}>{tr('detail.traceCumulative')}</h4>
...
function RequestOptions({
  ...
  if (options === undefined) {
    return <p className={css.noPayload}>{tr('detail.optionsNotRecorded')}</p>;
  }
  return (
    <JsonTree
      data={options}
...
function messageSourceLabel(source: unknown): string {
  if (typeof source !== 'object' || source === null || Array.isArray(source)) {
    return tr('detail.sourceUnknown');
  }
  const properties = source as Record<string, unknown>;
  const kind = properties.kind;
  if (kind === 'user') return tr('detail.sourceUser');
  if (kind === 'plugin') {
    const plugin = properties.plugin;
    return typeof plugin === 'string' && plugin !== ''
      ? `${tr('detail.sourcePlugin')} · ${plugin}`
      : tr('detail.sourcePlugin');
  }
  if (kind === 'goal') {
    const round = properties.round;
    return typeof round === 'number' && round > 0
      ? `${tr('detail.sourceGoal')} · ${tr('detail.round').replace('{n}', String(round))}`
      : tr('detail.sourceGoal');
  }
  if (typeof kind !== 'string' || kind === '') return tr('detail.sourceUnknown');
  return `${kind[0]?.toUpperCase() ?? ''}${kind.slice(1)}`;
}

function MessageSource({ record }: { record: TableRecord }) {
  const source = record.cell.messageSource;
  if (source === undefined) return <p className={css.noPayload}>{tr('detail.sourceNotRecorded')}</p>;
...
function detailTabs(record: TableRecord): readonly DetailTabItem[] {
  if (record.cell.kind === 'system') {
    return (record.cell.previousPromptDetail === undefined
      ? SYSTEM_PROMPT_TABS
      : SYSTEM_UPDATE_TABS
    ).map(tab => ({ ...tab, label: tr(`tabs.${tab.id === 'system-prompt' ? 'systemPrompt' : tab.id === 'tools' ? 'tools' : 'diff'}`) }));
  }
  if (record.cell.kind === 'compacted') {
    return [
      { id: 'overview', label: tr('tabs.summary') },
      { id: 'raw', label: tr('tabs.rawOutput') },
    ];
  }
  if (isMarkdownRecord(record)) {
    return [
      { id: 'overview', label: tr('tabs.summary') },
      { id: 'rendered', label: tr('tabs.preview') },
      { id: 'raw', label: tr('tabs.raw') },
      ...(record.cell.messageSource === undefined
        ? []
        : [{ id: 'source', label: tr('tabs.source') } as const]),
      ...(record.cell.observabilityCallId !== undefined
        ? [{ id: 'payloads', label: tr('tabs.payloads') } as const]
        : []),
    ];
  }
  return [
    { id: 'overview', label: tr('tabs.summary') },
    ...(record.cell.inputDetail ? [{ id: 'input', label: tr('tabs.payload') } as const] : []),
    ...(record.cell.outputDetail ? [{ id: 'output', label: tr('tabs.result') } as const] : []),
    ...(record.cell.schemaDetail ? [{ id: 'schema', label: tr('tabs.schema') } as const] : []),
    { id: 'timing', label: tr('tabs.timing') },
    ...(record.cell.observabilityCallId !== undefined
      ? [{ id: 'payloads', label: tr('tabs.payloads') } as const]
      : []),
  ];
...
function recordDisplayText(cell: TrajectoryCellProps): string {
  ...
  if (cell.kind === 'system') return tr(cell.text);
...
function isToolCallOnly(cell: TrajectoryCellProps): boolean {
  return cell.kind === 'message'
    && !cell.outputDetail
    && !cell.thinkingDetail
    && cell.text === tr('table.toolCallOnly');
}
...
function RecordListText({
  ...
  if (toolCallOnly) {
    return <span className={css.toolCallOnly}>({tr('table.toolCallOnly')})</span>;
  }
          ...
          {block.callId !== undefined
            ? (
              <button
                ...
                aria-label={tr('detail.openBlockToolCall').replace('{n}', String(index + 1))}
                title={tr('detail.openToolCallSummary')}
                onClick={() => {
                  if (block.callId !== undefined) onOpenCall(block.callId);
                }}
              >
                <span className={css.sourceBlockLabel}>
                  {`${tr('detail.block')} #${index + 1} ${block.type}`}
                </span>
                <span className={css.sourceBlockJumpIcon}>
...
function ToolCatalog({ tools }: { tools: TrajectoryPromptSnapshot['tools'] }) {
  if (tools.length === 0) return <p className={css.noPayload}>{tr('detail.noToolsInRequest')}</p>;
  return (
      ...
      {tools.map((tool, index) => (
        <details className={css.toolCatalogItem} key={`${tool.name}:${index}`}>
          <summary className={css.toolCatalogSummary}>
            <span className={css.toolCatalogChevron}><ChevronRightIcon size={12} /></span>
            <ToolGlyph />
...
function MarkdownRecordContent({
    ...
    return (
      <div className={`${css.assistantContent} ${css.assistantContentRendered}`}>
        ...
        >
          ...
          >
            {tr('detail.thinking')}
            <span className={css.thinkingChevron}><ChevronRightIcon size={12} /></span>
          </button>
          {thinkingExpanded && (
            <MarkdownFragment
  ...
  if (!source && !hasImages && !hasToolCalls) {
    const emptyLabel = isToolCallOnly(record.cell)
      ? tr('table.toolCallOnly')
      : record.cell.text || tr('detail.noContent');
...
function RecordTiming({ record }: { record: TableRecord }) {
  return record.cell.kind === 'message' && record.cell.assistantMetrics !== undefined
    ...
    : (
      <dl className={css.overview}>
        <div><dt>{tr('detail.started')}</dt><StartedAtValue timestamp={record.cell.startedAt ?? null} /></div>
        <div><dt>{tr('detail.duration')}</dt><dd>{formatElapsedSeconds(record.cell.timeSeconds)}</dd></div>
        <div>
          <dt>{tr('detail.timingSource')}</dt>
          <dd>{record.cell.timeSeconds === null
            ? tr('detail.notAvailable')
            : record.cell.timingSource === 'session'
              ? tr('detail.timing.session')
              : tr('detail.timing.observability')}</dd>
        </div>
      </dl>
    );
}
...
function RequestTiming({
  ...
  if (request?.startedAt !== undefined) {
    ...
    return (
      <dl className={css.overview}>
        <div><dt>{tr('detail.started')}</dt><StartedAtValue timestamp={request.startedAt} /></div>
        <div><dt>{tr('detail.duration')}</dt><dd>{formatElapsedSeconds(duration)}</dd></div>
        <div>
          <dt>{tr('detail.timingSource')}</dt>
          <dd>{duration === null ? tr('detail.timing.observabilityRunning') : tr('detail.timing.observability')}</dd>
        </div>
      </dl>
    );
  }
  return (
    <dl className={css.overview}>
      <div>
        <dt>{tr('detail.started')}</dt>
        <StartedAtValue timestamp={anchor?.cell.startedAt ?? null} />
      </div>
      <div><dt>{tr('detail.duration')}</dt><dd>{formatElapsedSeconds(null)}</dd></div>
    </dl>
  );
}
...
function RecordPayload({
  ...
  const missing = direction === 'input'
    ? tr('detail.noPayloadCaptured')
    : tr('detail.noResultCaptured');
  ...
  return (
    <pre className={[
      ...
      error ? css.errorPayload : undefined,
      value === tr('detail.noOutput') ? css.noOutputText : undefined,
    ].filter((value): value is string => value !== undefined).join(' ')}
    >
      {value}
    </pre>
...
function RecordSchema({
  ...
  if (!record.cell.schemaDetail) {
    return <p className={css.noPayload}>{tr('detail.schemaUnavailable')}</p>;
  }
    ...
    return (
      <div className={preview ? `${css.schema} ${css.schemaPreview}` : css.schema}>
        ...
        <section className={css.schemaParameters}>
          <h4 className={css.schemaParametersTitle}>{tr('detail.parameters')}</h4>
          <JsonTree
...
export function PayloadPlainView({ record, text, hideKindLabel = false }: {
  ...
  const shown = clamped ? text.slice(0, PAYLOAD_PLAIN_COLLAPSED_CHARS) : text;
  return (
    <section className={css.payloadView}>
      ...
      {text.length > PAYLOAD_PLAIN_COLLAPSED_CHARS && (
        <button
          type="button"
          className={css.payloadViewMore}
          aria-expanded={expanded}
          onClick={() => { setExpanded(current => !current) }}
        >
          {expanded ? tr('payloadView.less') : tr('payloadView.more')}
        </button>
      )}
    </section>
  );
...
/**
 * 灵犀扩展：「观测载荷」tab —— 打开即并行拉取该调用全部载荷正文，
 * 以 TXT 阅读式纯文本直出（自然分段，不出 JSON 语法、不出徽章元数据）。
 */
function ObservabilityPayloadsPanel({
  callId,
  isLocalOwner,
}: {
  callId: string;
  ...
  const [state, setState] = useState<
    ...
    | { status: 'loaded'; views: { record: ModelObservabilityPayloadRecordMetadata; text: string }[] }
  >({ status: 'loading' });
  useEffect(() => {
    ...
    loadObservabilityCallDetail(callId, { signal: controller.signal })
      .then(detail => {
        const records = detail.payloadRecords;
        if (records.length === 0) {
          setState({ status: 'loaded', views: [] });
          return;
        }
        return Promise.all(records.map(record => loadObservabilityPayloadRecord(record.id)))
          .then(details => {
            setState({
              status: 'loaded',
              views: records.map((record, index) => ({
                record,
                text: payloadToReadableText(details[index]?.payload ?? null),
              })),
            });
          });
      })
  ...
  if (state.status === 'loading') {
    return <p className={css.noPayload}>{t('settings.observability.loading.payload')}</p>;
  }
  if (state.status === 'error') {
    return <p className={css.noPayload} role="alert">{state.message}</p>;
  }
  if (state.views.length === 0) {
    return <p className={css.noPayload}>{tr('detail.payloadNotCaptured')}</p>;
  }
  void isLocalOwner;
  return (
    <div className={css.payloadViewList}>
      {state.views.map(view => (
        <PayloadPlainView key={view.record.id} record={view.record} text={view.text} />
      ))}
    </div>
  );
}
...
/**
 * 灵犀扩展：SYSTEM 记录的 System Prompt / Tools tab 内容。
 *
 * dsh 的 prompt 快照来自其引擎的请求头事件流；灵犀观测库只有在开启载荷
 * 捕获时才存 semantic_request（含 systemPrompt/tools）。未捕获 → 显式
 * 「未捕获」态，不伪造。
 */
function SystemPromptLazyPanel({
  callId,
  mode,
}: {
  callId: string;
  ...
  const [state, setState] = useState<
    ...
    | { status: 'error'; message: string }
    | { status: 'not-captured' }
    | { status: 'loaded'; systemPrompt: string | null; tools: { name: string; description: string; parameters: object }[] | null }
  >({ status: 'loading' });
  useEffect(() => {
    const controller = new AbortController();
    setState({ status: 'loading' });
    loadObservabilityCallDetail(callId, { signal: controller.signal })
      .then(detail => {
        const record = detail.payloadRecords.find(
          candidate => candidate.kind === 'semantic_request' && candidate.hasBody,
        );
        if (record === undefined) {
          setState({ status: 'not-captured' });
          return;
        }
        return loadObservabilityPayloadRecord(record.id)
          .then(payloadDetail => {
            const payload = payloadDetail.payload;
            ...
            const systemPrompt = typeof record.systemPrompt === 'string'
              ? record.systemPrompt
              : record.systemPrompt === null ? '' : null;
            const toolsRaw = Array.isArray(record.tools) ? record.tools : null;
            const tools = toolsRaw?.flatMap((tool): {
              ...
              const parameters = ['parameters', 'input_schema', 'function']
                .map(key => {
                  const value = key === 'function'
                    ? (candidate.function as Record<string, unknown> | undefined)?.parameters
                    : candidate[key];
                  return typeof value === 'object' && value !== null && !Array.isArray(value) ? value as object : undefined;
                })
            ...
            setState({ status: 'loaded', systemPrompt, tools });
          });
      })
      .catch((error: unknown) => {
        if (isObservabilityAbortError(error)) return;
  ...
  if (state.status === 'not-captured') {
    return <p className={css.noPayload}>{tr('detail.payloadNotCaptured')}</p>;
  }
  if (mode === 'system') {
    return state.systemPrompt === null
      ? <p className={css.noPayload}>{tr('detail.noSystemPrompt')}</p>
      : state.systemPrompt === ''
        ? <p className={css.noPayload}>{tr('detail.noSystemPrompt')}</p>
        : (
          <div className={`${css.markdownPayload} ${css.systemPrompt}`}>
            <MarkdownText text={state.systemPrompt} />
          </div>
        );
  }
  return state.tools === null
    ? <p className={css.noPayload}>{tr('detail.noToolsInRequest')}</p>
    : <ToolCatalog tools={state.tools} />;
}
  ...
  return (
    <section className={css.overviewSection}>
      <h3 className={css.overviewHeading}>
        ...
        >
          <span>{label}</span>
          <span className={css.overviewTitleIcon}><ChevronRightIcon size={12} /></span>
        </button>
      </h3>
      <div
...
/**
 * 渲染轨迹事件为带轮次与步骤分隔的紧凑台账。点击台账空白清除活动选择。
 */
export function TrajectoryTable({
  ...
  const selectedTabs = selectedRequest !== null
    ? REQUEST_TABS
      .map(tab => ({ ...tab, label: tr(`tabs.${tab.id === 'overview' ? 'summary' : tab.id}`) }))
      .filter(tab => tab.id !== 'options' || selectedRequestOptions !== undefined)
  ...
  // 跨视图 inspect 交接：解析请求的 callId 到其记录、打开摘要、记住待滚动行。
    ...
    <div ref={rootRef} className={css.split} style={splitStyle}>
                ...
                <td colSpan={2}>
                  <button
                    ...
                    aria-label={olderBusy
                      ? tr('table.loadingEarlier')
                      : tr('table.loadEarlier')}
                    onClick={() => {
                      const pane = tablePaneRef.current;
                  ...
                  >
                    ...
                    <span aria-hidden="true">
                      {olderBusy ? tr('table.loadingEarlier') : tr('table.loadEarlier')}
                    </span>
                    <span className={css.visuallyHidden} role="status" aria-live="polite">
                      {olderBusy ? tr('table.loadingEarlier') : ''}
                    </span>
                  </button>
                </td>
              </tr>
              ...
              >
                ...
                ?? (record.cell.isError === true ? 'error' : undefined);
                  ...
                  const requestLabel = request === undefined
                    ? undefined
                    : `${tr('table.request').replace('{n}', String(request))}${requestInfo?.purpose === 'side' ? ` · ${tr('table.requestSide')}` : ''}`;
                  ...
                  return (
                    <tr
                      ...
                      aria-label={isCollapsedSummary
                        ? `${tr('table.collapsedSummaryAria')} ${record.collapsedSummary}`
                        : isRequestOnly
                          ? `${tr('table.request').replace('{n}', String(request ?? ''))}, ${tr('table.requestSide')}`
                          : `${request === undefined ? '' : `${tr('table.request').replace('{n}', String(request))}, `}${kindLabelOf(record.cell.kind)}, ${listDisplayText || tr('detail.noContent')}`}
                        ...
                        <div className={css.eventInner}>
                          {!isCollapsedSummary && !isRequestOnly && (
                            ...
                            >
                              ...
                              >
                                <Tooltip
                                  content={kindLabelOf(record.cell.kind)}
                                  placement="right"
                                >
                              ...
                              >
                                ...
                                {resultText !== undefined && (
                                  <span className={record.cell.isError ? `${css.inlineResult} ${css.error}` : css.inlineResult}>
                                    ...
                                    <span className={resultText === tr('detail.noOutput')
      ...
      {(selectedRequest !== null
        ...
        >
                ...
                ? (
                  <>
                    ...
                    <span className={css.requestDetailsName}>
                      {`${tr('table.request').replace('{n}', String(selectedRequestNumber ?? '—'))}`}
                    </span>
                    <span className={css.detailsLocation}>
                      {selectedRequestInfo?.purpose === 'side'
                        ? `${tr('table.requestSide')} · ${sectionLabel(selectedRequest.turn)}`
                ...
                : promptSelected
                  ...
                  : selected !== undefined && (
                    <>
                      ...
                      >
                        {kindLabelOf(selected.cell.kind)}
                      </span>
                      <span className={css.detailsLocation}>
                        {selected.cell.kind === 'compacted'
          ...
          <div className={css.detailTabs} role="tablist" aria-label={tr('detail.aria')}>
          ...
          >
            {selectedRequest !== null
              ...
              <>
                ...
                >
                  ...
                  {selectedRequestInfo?.purpose === 'side' && (
                    <div>
                      <dt>{tr('detail.purpose')}</dt>
                      <dd>{tr('table.requestSide')}</dd>
                    </div>
                  )}
                  {selectedRequestInfo?.provider !== undefined && (
                    <div>
                      <dt>{tr('detail.provider')}</dt>
                      <dd>{selectedRequestInfo.provider}</dd>
                    </div>
                  )}
                  {selectedRequestInfo?.model !== undefined && (
                    <div>
                      <dt>{tr('detail.model')}</dt>
                      <dd>{selectedRequestInfo.model}</dd>
                    </div>
                  )}
                  <div>
                    <dt>{tr('detail.toolCalls')}</dt>
                    <dd>{selectedRequestToolCalls}</dd>
                  </div>
                  {selectedRequestSubtoolCalls > 0 && (
                    <div>
                      <dt>{tr('detail.subtoolCalls')}</dt>
                      <dd>{selectedRequestSubtoolCalls}</dd>
                    </div>
                  )}
                  {selectedRequestInfo?.error !== undefined && (
                    <div>
                      <dt>{tr('detail.error')}</dt>
                      <dd className={css.error}>{selectedRequestInfo.error}</dd>
                    </div>
                  )}
                  {selectedRequestResult !== undefined && (
                    <div>
                      <dt>{tr('detail.result')}</dt>
                      <dd className={css.overviewParentLinks}>
                        ...
                        >
                          <span>{tr('detail.assistantMessage')}</span>
                          <span className={css.overviewHierarchyJumpIconTight}>
                ...
                <div className={css.overviewSections}>
                  {selectedRequestOptions !== undefined && (
                    <OverviewSection label={tr('tabs.options')} onOpen={() => { activateTab('options') }}>
                      <RequestOptions options={selectedRequestOptions} preview />
                    </OverviewSection>
                  )}
                  <OverviewSection label={tr('tabs.usage')} onOpen={() => { activateTab('usage') }}>
                    <UsageRows usage={selectedRequestUsage} />
                  </OverviewSection>
                  <OverviewSection label={tr('tabs.timing')} onOpen={() => { activateTab('timing') }}>
                    <RequestTiming
            ...
            {promptSelected && activeTab === 'system-prompt' && (
              selectedPrompt !== undefined && selectedPrompt.system === ''
                ? <p className={css.noPayload}>{tr('detail.noSystemPrompt')}</p>
                : (
                  <div className={`${css.markdownPayload} ${css.systemPrompt}`}>
            ...
            {promptSelected && activeTab === 'tools' && (
              selectedPrompt !== undefined && <ToolCatalog tools={selectedPrompt.tools} />
            )}
            {!promptSelected
              && selected?.cell.kind === 'system'
              && activeTab === 'system-prompt'
              && (
                selected.cell.observabilityCallId !== undefined
                  ? <SystemPromptLazyPanel callId={selected.cell.observabilityCallId} mode="system" />
                  : <p className={css.noPayload}>{tr('detail.payloadNotCaptured')}</p>
              )}
            {!promptSelected
              && selected?.cell.kind === 'system'
              && activeTab === 'tools'
              && (
                selected.cell.observabilityCallId !== undefined
                  ? <SystemPromptLazyPanel callId={selected.cell.observabilityCallId} mode="tools" />
                  : <p className={css.noPayload}>{tr('detail.noToolsInRequest')}</p>
              )}
            {!promptSelected
              && selected?.cell.kind === 'compacted'
              ...
              <>
                    ...
                    <div>
                      ...
                      <dd className={css.overviewParentLinks}>
                        {selectedAssistantRequestTarget !== undefined && (
                          ...
                          >
                            <span>{tr('table.request').replace('{n}', String(selectedAssistantRequest ?? '—'))}</span>
                            <span className={css.overviewHierarchyJumpIconTight}>
                              <ChevronRightIcon size={11} />
                            </span>
                          </button>
                ...
                <div className={css.overviewSections}>
                  {isMarkdownRecord(selected)
                    ? (
                      <>
                        <OverviewSection label={tr('tabs.preview')} onOpen={() => { activateTab('rendered') }}>
                          <MarkdownRecordContent
                            record={selected}
                            rendered
                            preview
                    ...
                    : (
                      <>
                        {selected.cell.inputDetail && (
                          <OverviewSection label={tr('tabs.payload')} onOpen={() => { activateTab('input') }}>
                            <RecordPayload record={selected} direction="input" preview />
                          </OverviewSection>
                        )}
                        {selected.cell.outputDetail && (
                          <OverviewSection label={tr('tabs.result')} onOpen={() => { activateTab('output') }}>
                            <RecordPayload record={selected} direction="output" preview />
                          </OverviewSection>
                        )}
                        {selected.cell.schemaDetail && (
                          <OverviewSection label={tr('tabs.schema')} onOpen={() => { activateTab('schema') }}>
            ...
            {!promptSelected
              ...
              && activeTab === 'payloads'
              && selected.cell.observabilityCallId !== undefined && (
              <ObservabilityPayloadsPanel
                callId={selected.cell.observabilityCallId}
