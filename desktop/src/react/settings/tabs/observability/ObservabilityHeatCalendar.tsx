/**
 * ObservabilityHeatCalendar.tsx — 「调用热力日历」（滚动 12 个月）。
 *
 ...
 * 悬停显示对应口径的调用次数 + Token 总量（每日档标注“当日调用次数与 Token 用量”）。
 */
...
export function ObservabilityHeatCalendar({ refreshToken }: { refreshToken: number }) {
  ...
  const onHover = (index: number, evt: React.MouseEvent) => {
    ...
    if (mode === 'weekly') {
      ...
      title = `${formatFullLocalDate(day.date)} · ${t('settings.observability.charts.heat.tooltipHint')}`;
      calls = day.calls;
      tokens = day.tokens;
    }
    // 锚定所在格子：提示框放格子下/上方，所指行始终可见。
    let anchor: ChartTip['anchor'];
    ...
    if (svg && card) {
      ...
      const cell = svgAnchor(svg, card, VIEW_W, cellX + cellSide / 2, cellY + cellSide / 2);
      anchor = {
        cx: cell.cx,
        cy: cell.cy,
        h: Math.max(cellSide * cell.scale, 8),
        placement: 'vertical',
        fx: evt.clientX - card.getBoundingClientRect().left,
      };
    }
    setTip({
      title,
