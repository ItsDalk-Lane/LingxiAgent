...
export function MediaProviderDetail({ providerId, provider, capability = 'imageGeneration', config, onSaveConfig, onRefresh }: Props) {
  ...
  const addModel = async (modelId: string) => {
    try {
      ...
      await lingxiFetch(`/api/media/${mediaRoute}/providers/${encodeURIComponent(providerId)}/models`, {
        method: 'POST',
  ...
  const removeModel = async (modelId: string) => {
    try {
      await lingxiFetch(`/api/media/${mediaRoute}/providers/${encodeURIComponent(providerId)}/models/${encodeURIComponent(modelId)}`, {
        method: 'DELETE',
      });
      invalidateConfigCache();
      await onRefresh();
