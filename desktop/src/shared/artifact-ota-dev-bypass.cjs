 ...
 *
 * `LINGXI_ARTIFACT_MANIFEST=<path|url>` lets a developer rehearse the full
 ...
 * unconditional alias entry there) — the string "LINGXI_ARTIFACT_MANIFEST"
 ...
 * reference `process.env.LINGXI_ARTIFACT_MANIFEST` from any other file in
 * the OTA module — this file is the ONLY place that name may appear.
 */

function resolveDevManifestOverride() {
  const value = process.env.LINGXI_ARTIFACT_MANIFEST;
